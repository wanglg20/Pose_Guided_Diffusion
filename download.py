...
def process(data, seq_id, videoname, output_root):
    ...
    for pngname in png_list:
        ...
        if int(image.shape[1]/2) < 500:
            break
        #image = misc.imresize(image, (int(image.shape[0]/2), int(image.shape[1]/2)), interp='bilinear')
        #image = np.array(Image.fromarray(image).resize((int(image.shape[1]/2), int(image.shape[0]/2)), resample=Image.BILINEAR))
        
        #record resolution
        with open('resolution_record.txt', 'w') as f:
            img_size = image.shape
            f.write(pngname + str(img_size[0]) + ' ' + str(img_size[1]))
            f.close()
...
class DataDownloader:
    def __init__ (self, dataroot, mode='test'):
        print("[INFO] Loading data list from "+dataroot,end='')
        self.dataroot = dataroot
        self.list_seqnames = sorted(glob.glob(dataroot + '/*.txt'))
        # sample only 10 videos
        self.list_seqnames = self.list_seqnames[:5]
        
        self.output_root = './dataset/RealEstate_raw_data/' + mode + '/'
        self.mode =  mode
    ...
    def Run(self):
        ...
        for global_count, data in enumerate(self.list_data):
            ...
            try :
                ...
                print("[INFO] Downloading {} is done".format(data.url))
            except :
...
#
    ...
    mode = "test"
    dataroot = "dataset/RealEstate10K/" + mode
    ...
    list_seqnames = sorted(glob.glob(dataroot + '/*.txt'))